...
- [ ] Paste the real Meta Pixel base code into the placeholder comments in `index.html` and `public/thank-you.html`.
...
- [ ] Paste the real Google Tag base code into the placeholder comments in `index.html` and `public/thank-you.html`.
...
- [ ] Replace `REPLACE_WITH_GTAG_ID` in `src/components/lead-form.js` (`GTAG_CONVERSION_ID` constant) with the same real conversion label.
...
- [ ] Submit the signup form for real on the live (deployed) site — not just locally — and confirm: the lead appears in the `leads` table, the welcome email arrives, and the redirect to `/thank-you` works.
...
- [ ] Load the live site with `?utm_source=test&utm_campaign=test` in the URL and confirm those values carry through into the Skool link (and, on `/thank-you`, into the Skool button) and into the stored lead entity in Table Storage.
