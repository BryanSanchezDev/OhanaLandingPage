...
`public/thank-you.html`, `public/privacy-policy.html`, and `public/terms.html` are plain, self-contained HTML files — not Vite components, no shared styles/tokens, no build step, and (since they're outside the Vite bundle) no access to `src/` modules. Vite copies `public/` to the build output untouched, and Azure Static Web Apps' clean-URL routing serves them at `/thank-you`, `/privacy-policy`, `/terms` (no `.html` needed). `privacy-policy.html` and `terms.html` currently have an HTML comment placeholder where the Termly-generated legal content goes.

`thank-you.html` is the post-signup redirect target and where conversion pixels actually fire (on its own page load), not in `lead-form.js`. Because it's a standalone document, `main.js`'s `cookieConsentGranted` listener never runs on it — that CustomEvent only fires in-memory during the SPA's own page lifetime. What *does* carry over across the navigation is the consent choice itself in `localStorage`, so `thank-you.html` re-implements a small inline copy of `cookie-banner.js`'s `hasMarketingConsent()` logic (same `ohana_cookie_consent` key and shape) directly in its own `<script>`, gating both the placeholder pixel base-code slots and the actual tracking calls. If you ever change the consent storage key/shape in `cookie-banner.js`, update this inline copy in `thank-you.html` too — they're intentionally duplicated, not shared, the same tradeoff already made for UTM handling (see `utils/utm.js` vs. the inline duplicate in `thank-you.html`).
...
- Single paid tier: $9.99/month all-access membership, no free tier (annual option: $99/year). Pricing/plan copy currently lives in both `src/components/features.js` (feature card) and `src/components/pricing.js` (dedicated pricing section) — keep these two in sync when pricing changes. The AI consultant chatbot is branded **Bella** throughout site copy.
- "Start for $9.99/month" CTAs across the page do **not** link directly to Skool — they call `onclick="window.openLeadModal()"` to open the shared signup modal. On successful submission, the modal redirects to `/thank-you`, which tells the visitor to check their email for their Skool join link (sent via the welcome email in `api/subscribe`) — there is no direct "Join on Skool" button or auto-redirect on that page. There is no checkout page of ours and no payment-completion webhook back from Skool, so conversion pixels (which fire on `/thank-you`'s page load, see above) reflect form submissions, not verified payment — see the caveat comment in `public/thank-you.html`. The footer's "Join the Community" link is kept as an `<a href="#" onclick="window.openLeadModal(); return false;">` rather than a `<button>`, since its styling comes from a `.footer-links a` element selector.
